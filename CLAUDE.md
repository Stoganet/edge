...
- **Adding a public hostname** requires *both* a Caddy block in `compose/Caddyfile` *and* a conscious decision to widen the public surface. Most services should stay NetBird-only (see `docs/architecture.md` §"Why everything else stays NetBird-only").
